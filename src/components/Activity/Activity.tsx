...
export const Activity = ({ gameImageUrl, name, onClick }: ActivityProps) => {
  return (
    <div className="aspect-square bg-arctic-500 cursor-pointer flex flex-col group overflow-hidden relative rounded-md select-none" onClick={onClick}>
      <div
      ...
      <div className="absolute bg-arctic-500 bottom-0 duration-300 font-round2 font-semibold h-8 p-1 rounded-b text-white text-center transition-all translate-y-full group-hover:translate-y-0 w-full">
        Details ansehen
      </div>
    </div>
  );
