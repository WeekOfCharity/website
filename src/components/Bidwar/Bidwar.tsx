...
export const Bidwar = ({ name, description, options, status = "active", timeslot = undefined }: BidwarProps) => {
  ...
  return (
    <div className="relative flex flex-col font-bold max-w-screen-lg w-full">
      {timeslot && getState(timeslot.start, timeslot.end) === "ended" && <GoalStamp className="absolute text-blue23-700 -top-4 -right-4 rotate-[15deg] w-40 md:w-56 z-[1]"/>}
      <div className="bg-blue23-200">
        <div className="relative py-7 px-2 md:px-4">
          ...
          <h2 className="relative text-blue23-800 text-2xl md:text-3xl text-center mt-3 md:mt-6 mx-6 z-[1]">{name}</h2>
          <div className="flex flex-row items-center justify-center mt-2 mb-6 z-[1]">
            <h3 className={classNames("text-blue23-800 text-center z-[1]", {
            ...
            {status === "active" && description && (
              <div className="relative font-normal text-md md:text-lg flex justify-center items-center z-[3]">
                <button className="cursor-default py-2"
                ...
                >
                  <Icon className={classNames("ml-2", {"text-blue23-600": descriptionShown, "text-blue23-800": !descriptionShown})} path={mdiInformationOutline} size="2rem"/>
                ...
                {descriptionShown && (
                  ...
                  <div className="absolute cursor-auto select-text top-16 -left-[296px] -right-[33px] md:-left-[446px] md:-right-[133px] bg-blue23-700 text-blue23-200 shadow-2xl rounded-lg p-6">{description}</div>
                  </>
                )}
                </button>
              </div>
        ...
        {sortedOptionKeys && sortedOptionKeys.length > 3 && (
          ...
          <button className="group flex justify-center items-center text-blue23-700 w-32 h-12" type="button" onClick={toggleExpanded}>
            <RoundArrow className={classNames("text-blue23-700 w-14 h-16 group-hover:text-blue23-500 transition-transform duration-300", {
              "-rotate-90" : expanded,
              "rotate-90" : !expanded,
            })}/>
          </button>
        ...
        <div className="p-5 pt-0 isolate">
          <div className='font-round2 font-bold mb-1 text-blue23-900 isolate'>Sei bei der Umsetzung dabei</div>
          {timeslot && (
            <Stream
